...
SELECT name, monthlymaintenance, CASE WHEN monthlymaintenance > 100 THEN 'expensive'
    ELSE 'cheap' END AS expense_description
...
SELECT firstname, surname, joindate FROM country_club.Members
...
SELECT DISTINCT(members.memid), CONCAT(members.surname, ', ', members.firstname) AS member_name, facilities.name AS court_name FROM country_club.Members members
...
SELECT CONCAT(members.surname, ', ', members.firstname) AS member_name, facilities.name as facility_name, 
...
SELECT CONCAT(members.surname, ', ', members.firstname) AS member_name, sub.name as facility_name, sub.total_cost
FROM (
    SELECT bookings.facid, bookings.memid, facilities.name, bookings.slots, 
    CASE WHEN bookings.memid = 0 THEN bookings.slots*facilities.guestcost
        ELSE bookings.slots*facilities.membercost END AS total_cost 
        FROM country_club.Bookings bookings
        LEFT JOIN country_club.Facilities facilities
...
SELECT facilities.name AS facility_name, SUM(CASE WHEN bookings.memid = 0 THEN bookings.slots*facilities.guestcost
